...
code_hunk * assign_lexpr(lexpr_hunk *lh, const char *assignop, code_hunk *ch_r) {
        ...
        if(offset_ch != NULL) {
            ...
            if(subscript_ch == NULL) {
                ...
                if(l_op > 0) {
                    ...
                    ret = CCH5(ret, get_empty_block(), CHS("stack[offset]"), new_sym_thing(op), call_send(1));
                }
                ...
                if(l_op > 0) {
                    ...
                    ret = CCH3(ret, new_sym_thing(op), call_send(1));
                }
    ...
    if(subscript_ch == NULL) {
        ...
        if(l_op > 0) {
            ...
            ret = CCH5(ret, get_empty_block(), CHS("stack[sp] = stack[sp-4]; sp++;\n"), new_sym_thing_from_symbol(sym), call_send(0));
        ...
        ret = CCH3(ret, new_sym_thing(setter_str), call_send(1));
        ...
        ret = CCH5(ret, get_empty_block(), self_ch, new_sym_thing_from_symbol(sym), call_send(0));
        ...
        if(l_op > 0) {
            ...
            /* Now make the subscript call. We have the subscript, add the empty block, the local var, and the symbol */
            ret = CCH5(ret, get_empty_block(), CHS("stack[sp] = stack[sp-6]; sp++;\n"), new_sym_thing("[]"), call_send(1));
        ...
        ret = CCH6(ret, CHS("stack[sp] = stack[sp-2]; sp++;\n"), get_empty_block(), CHS("stack[sp] = stack[sp-4]; sp++;\n"), new_sym_thing("[]="), call_send(2));
